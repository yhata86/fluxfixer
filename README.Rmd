...
<!-- badges: start -->
[![CRAN status](https://www.r-pkg.org/badges/version/fluxfixer)](https://CRAN.R-project.org/package=fluxfixer)
...
[![Codecov test coverage](https://codecov.io/gh/yhata86/fluxfixer/graph/badge.svg)](https://app.codecov.io/gh/yhata86/fluxfixer)
<!-- badges: end -->
...
``` {r example_lib, echo = FALSE, message = FALSE, warning = FALSE, results = FALSE}
library(fluxfixer)
library(ggplot2)
library(dplyr)
library(ggtext)
...
## For x-axis label
df_label <-
  data.frame(date = seq(lubridate::ymd("2012/09/01", tz = "Etc/GMT-8"),
                        lubridate::ymd("2013/08/01", tz = "Etc/GMT-8"),
                        by = "1 month")) %>% 
  dplyr::mutate(date = date + lubridate::days(15),
                yval = -1,
                label_mon = c(month.abb[9:12], month.abb[1:8]))
...
dt_noisy %>% 
  ...
  scale_x_datetime(date_breaks = "1 month", expand = expansion(0.02, 0))+
  scale_y_continuous(limits = c(-1, 22), breaks = seq(0, 20, by = 5),
                     labels = c(0, "", 10, "", 20), expand = expansion(0, 0))+
  geom_point(aes(time - 1800, dt), col = "royalblue")+
  geom_text(data = df_label, aes(date, yval, label = label_mon), size = 5,
            col = "black", vjust = 1.6)
```
...
## For x-axis label
df_label <-
  data.frame(date = seq(lubridate::ymd_hm("2013/02/01 12:00", tz = "Etc/GMT-8"),
                        lubridate::ymd_hm("2013/02/28 12:00", tz = "Etc/GMT-8"),
                        by = "2 day"),
             yval = 0,
             label_day = seq(1, 27, by = 2))
...
result %>% 
  ...
  theme(axis.title.x = element_text(size = 18, vjust = -3.5),
  ...
  coord_cartesian(clip = "off")+
  scale_x_datetime(limits = c(time_head, time_tail), date_breaks = "1 day",
  ...
  geom_text(data = df_label, aes(date, yval, label = label_day), size = 5,
            col = "black", vjust = 1.6)+

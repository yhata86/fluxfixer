...
\`calc_dtmax_dr()\` calculates the time series of dTmax (the maximum
...
\* The third column, \`dtmax_dr\`, gives the estimated dTmax by the
double regression method. If \`output_daily\` is \`FALSE\` (default),
this column has the same time step as the input timestamp. If
\`output_daily\` is \`TRUE\`, the dTmax is returned in daily steps.

## Details

The double regression method is one of the methods for determining
dTmax. This method first calculates the moving window mean value of
dTmax, estimated by the daily predawn method, with an eleven-day length.
The dTmax that is lower than the mean is omitted, and then the moving

...
\`rf_fit()\` conducts an out-of-bag evaluation for hyperparameters used
in constructing the random forest model using a grid search approach.
...
\* The first column, \`min_nodesize\`, gives one of the
\`vctr_min_nodesize\` hyperparameter values tested in each model
construction during out-of-bag evaluation.

\* The second column, \`m_try\`, gives one of the \`vctr_m_try\`
...
\* The fourth column, \`MSE_OOB\`, provides the mean squared error

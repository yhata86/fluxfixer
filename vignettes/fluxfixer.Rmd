...
Output values are obtained from 500 (default; specified by `n_tree`) trees, and the first quartile (Q1), third quartile (Q3), and interquartile range (IQR) of the output values at each time point are calculated.
If the targeted value is less than Q1 − 1.5IQR or more than Q3 + 1.5IQR (default), the data point is identified as an outlier and removed. The coefficient 1.5 can be changed by `coef_iqr`.
